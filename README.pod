...
I<lemonbar> [-h | -g I<width>B<x>I<height>B<+>I<x>B<+>I<y> | -r | -b | -d | -f I<font> | -p | -n I<name> | -u I<pixel> | -B I<color> | -F I<color> | -U I<color> | -L I<color> | -o]
...
Sets the overline width in pixels. The default is 1.
...
Set the overline color of the bar. Accepts the same color formats as B<-B>.

=item B<-o> I<pixel>

Add a vertical offset to the text, it can be negative.
...
Set the bar background colour overriding the I<-B> flag, this must be at the beginning of the string. The parameter I<color> can be a color in one of the formats mentioned before.
...
Set the text overline color. The parameter I<color> can be I<-> or a color in one of the formats mentioned before. The special value I<-> resets the color to the default one.
...
Unique ids for hover enter and leave can be provided as:

Eg. I<%{A6,1:reboot:} Hover here to reboot %{A}>
